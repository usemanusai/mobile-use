        ...
        try:
            ...
            async for chunk in (await get_graph(context)).astream(
                ...
                if stream_mode == "updates":
                    for node_name, value in payload.items():  # type: ignore node name, node output
                        try:
                            await broadcaster.publish({"type": "update", "node": node_name})
                        except Exception:
                            pass
            ...
            try:
                await broadcaster.publish({"type": "task_end", "task_id": task.id})
            except Exception:
                pass
