...
# Determine Web GUI port and open the browser
$port = if ($env:WEB_GUI_PORT) { [int]$env:WEB_GUI_PORT } else { 8086 }
Start-Process "http://localhost:$port" | Out-Null

# Default to serving the Web GUI; allow overrides via extra args
if (-not $RemainingArgs -or $RemainingArgs.Count -eq 0) {
    $RemainingArgs = @("serve")
}

# Expose service ports for the Web GUI
docker compose run --service-ports --build --rm --remove-orphans -it mobile-use-full-ip $RemainingArgs
